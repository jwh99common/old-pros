(function () {
  function bindCTAFormcarry() {
    ...
    // Submit handler — send to Formcarry
    form.addEventListener("submit", async (e) => {
      e.preventDefault();
      const data = new FormData(form);

      try {
        const res = await fetch("https://formcarry.com/s/SiBMQMdNFbC", {
          method: "POST",
          body: data,
  ...
  if (placeholder) {
    const mo = new MutationObserver(() => {
      if (bindCTAFormcarry()) mo.disconnect();
    });
